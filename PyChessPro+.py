...
╠══════════════════════════════════════════════════════════════╣
    ...
    def cmd_redo(self, args):
        ...
        if len(self.app.redo_stack) > 0:
            self.app.redo_move()
            success_msg = "Ходът е повторен" if language == "bg" else "Move redone"
            self.print_text(success_msg, "success")
        else:
            warning_msg = "Няма ходове за повторение" if language == "bg" else "No moves to redo"
            self.print_text(warning_msg, "warning")
...
© 2024 PyChess Pro+ Team
    ...
    def redo_move(self):
        if self.redo_stack:
            ...
            self.current_move_number = min(len(self.game_board.move_stack), self.current_move_number + 1)
            # Update chart
            self.game_chart.update()
        else:
            QMessageBox.information(self, "Повторение" if self.language == "bg" else "Redo", 
                                  "Няма ходове за повторение." if self.language == "bg" else "No moves to redo.")

    def redo_move_old(self):
